<img src="logo.svg" width="76" align="left" alt="">

# Onionmind

**An uncensored LLM that lives on your machine and searches through Tor.**

<br clear="left">

Runs Qwen3.8-27B — abliterated, so it doesn't refuse — entirely on a 12GB consumer GPU, with
optional web search routed over Tor. Nothing but searches ever leaves the machine.

The name is the shape of the thing: onion-routed on the outside, a mind that stays local at the
centre. The mark draws it literally — each ring is both an onion layer and a circuit hop, gaps
rotating inward to trace a route down to the core that never leaves.
...
`onionmind.py` is pointed at whichever one was installed.
...
not Tails; it's Tails' mechanics with the model added — fail-closed tor-only
networking, clock sync over Tor, MAC randomization, amnesia — built with
`live-build` rather than Tails' own KVM-only toolchain. See **`usb/README.md`**.
...
- ❌ **Shell history captures one-shot queries.** `onionmind.py "sensitive question"` puts that
...
You asked about Parrot, so let me be straight about where it sits.
