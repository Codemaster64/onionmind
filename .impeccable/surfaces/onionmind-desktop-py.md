...
| --- | --- | --- |
| Native top toolbar | Repository, branch, model service, model tier, and one native Tor button showing the verified binary state and providing its action stay visible | Qt Widgets layouts and controls |
...
- Tor uses one visually explicit native button, translating the selected circular power-target reference into a 36px state target plus label inside the same persistent bordered button surface. Off and on show one complete power mark on one solid disc; verification replaces both with a single amber spinner on a quiet halo so rings never stack. The label says only `Tor is on` or `Tor is off`, and its tooltip and accessible name carry Turn on, Cancel, Retry, or Turn off. Disconnecting from an external proxy never claims to terminate that outside process.
