...
if [ "$DISTRO" = arch ]; then
  ...
  PKGS=(tor python-requests python-pysocks python-tk curl nodejs npm "$OLLAMA_PKG")
  MISSING=()
  ...
  sudo apt-get update
  sudo apt-get install -y tor python3-requests python3-socks python3-tk python3-venv curl ca-certificates nodejs npm \
...
# Onionmind Agent uses one pinned Adapter version. Debian-family repositories
# can still ship an older Node.js, so upgrade that runtime before npm installs
# the Adapter. The generated launcher repeats these checks before every run.
NODE_MAJOR=$(node --version 2>/dev/null | sed -n 's/^v\([0-9][0-9]*\).*/\1/p')
if [ "${NODE_MAJOR:-0}" -lt 22 ]; then
  [ "$DISTRO" = debian ] || die "Onionmind Agent requires Node.js 22 or newer"
  node_setup=$(mktemp) || die "could not create a Node.js setup temp file"
...
[ "${NODE_MAJOR:-0}" -ge 22 ] || die "Onionmind Agent requires Node.js 22 or newer"
command -v npm >/dev/null 2>&1 || die "npm is required to install Onionmind Agent"
say "Installing Onionmind Agent runtime"
sudo npm install --global @qwen-code/qwen-code@0.22.0 --no-fund --no-audit
command -v qwen >/dev/null 2>&1 || die "Onionmind Agent runtime is unavailable after installation"
...
case "$QWEN_VERSION" in
  0.22.0*) ;;
  *) die "Onionmind Agent runtime $QWEN_VERSION was installed; expected 0.22.0" ;;
esac
...
NODE_VERSION=\$(node --version 2>/dev/null) || {
  echo 'Onionmind Agent requires Node.js 22 or newer. Re-run Onionmind Setup.' >&2
  exit 1
}
...
command -v qwen >/dev/null 2>&1 || {
  echo 'Onionmind Agent runtime is missing. Re-run Onionmind Setup.' >&2
  exit 1
}
...
case "\$QWEN_VERSION" in
  0.22.0*) ;;
  *) echo "Onionmind Agent runtime is out of date (\$QWEN_VERSION). Re-run Onionmind Setup." >&2; exit 1 ;;
esac
...
echo "  Coding:      onionmind-code \"task\"   (local project edits; shell and web disabled)"
