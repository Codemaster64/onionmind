...
if [ "$DISTRO" = arch ]; then
  ...
  sudo apt-get update
  sudo apt-get install -y tor python3-requests python3-socks python3-tk curl ca-certificates
...
if [ "$#" -eq 0 ]; then
  exec python3 "$DIR/onionmind.py" --ui
else
...
sudo curl -fsSL https://raw.githubusercontent.com/Codemaster64/onionmind/main/dsh-onionmind-tor-search.js \
  -o "$DIR/dsh-onionmind-tor-search.js"
sudo curl -fsSL https://raw.githubusercontent.com/Codemaster64/onionmind/main/dsh-onionmind-tor.patch.yml \
...
#!/bin/sh
exec curl -fsSL https://raw.githubusercontent.com/Codemaster64/onionmind/main/update-onionmind.sh | sh -s -- --install-dir @DIR@
UPDATE
sudo sed -i "s|@DIR@|$DIR|" /usr/local/bin/onionmind-update
...
echo "  Coding:      onionmind-code   (DeepSeek Harness via Ollama)"
