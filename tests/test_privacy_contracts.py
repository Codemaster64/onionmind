...
class BackgroundTorTests(unittest.TestCase):
    ...
    def test_hidden_start_runs_tor_exe_without_a_window(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            startupinfo = mock.Mock(dwFlags=0, wShowWindow=None)
            with mock.patch.object(onionmind.os, "name", "nt"), \
                 ...
                 mock.patch.object(onionmind.subprocess, "STARTUPINFO", return_value=startupinfo, create=True), \
                 mock.patch.object(onionmind.subprocess, "STARTF_USESHOWWINDOW", 0x80, create=True), \
                 mock.patch.object(onionmind.subprocess, "SW_HIDE", 0, create=True), \
            ...
            self.assertEqual(startupinfo.dwFlags, 0x80)
...
class DistributionPrivacyTests(unittest.TestCase):
    ...
    def test_no_cloud_workflow_files_remain(self) -> None:
        ...
        self.assertLessEqual(present, allowed)
